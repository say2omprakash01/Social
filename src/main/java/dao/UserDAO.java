...
public class UserDAO {
    ...
    public String register(User user) {
        try {
            ...
            st.setString(1, user.getFirst_name());  // Updated method name
            st.setString(2, user.getLast_name());   // Updated method name
    ...
    public ArrayList<User> getUserByName(String name) throws SQLException {
        ...
        Connection conn = DBConnection.getInstance().getConnection();
        PreparedStatement st = conn.prepareStatement("SELECT * FROM user WHERE first_name LIKE ? OR last_name LIKE ?");
        st.setString(1, "%" + name + "%");
        st.setString(2, "%" + name + "%");
        ResultSet rs = st.executeQuery();
        while (rs.next()) {
            ...
            user.setPassword(rs.getString("password")); // Ensure password handling is secure
